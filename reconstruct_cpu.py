...
import losses as L
from baselines import utils as utls
...
def inversion(args, G, T, E, iden, itr, lr=2e-2, iter_times=1500, num_seeds=5):
    save_img_dir = os.path.join(args.save_dir, args.model, 'all_imgs')
    success_dir = os.path.join(args.save_dir, args.model, 'success_imgs')
...
if __name__ == "__main__":
    ...
    utls.load_state_dict(G, gen_ckpt)
    G = G.to(device)
    ...
    elif args.model == 'VGG_MixedGhost':
        T = VGG16_bn(2)
        path_T = 'checkpoints/target_model/ghost_mixed.tar'
    elif args.model == 'VGG_NONDP':
        T = VGG16_bn(2)
    ...
    ckp_E = torch.load(path_E, map_location=device)
    E.load_state_dict(ckp_E['state_dict'], strict=False)

    logger.info("=> Begin attacking ...")
    aver_acc, aver_acc5, aver_var, aver_var5 = 0, 0, 0, 0
    for i in range(1):
        # Attack 2 classes per batch since the model has 2 classes
        iden = torch.from_numpy(np.arange(2)).to(device)  # chỉ có 2 lớp

        # Evaluate on the first 2 identities only
        for idx in range(1):  # chỉ lặp 1 lần
            print("--------------------- Attack batch [%s]------------------------------" % idx)
            # Reconstructed private images
            acc, acc5, var, var5 = inversion(args, G, T, E, iden, itr=i, lr=args.lr, iter_times=args.iter_times,
                                             num_seeds=5)

            # Move to the next batch of classes
            iden = iden + 2
            aver_acc += acc / 5
            aver_acc5 += acc5 / 5
            aver_var += var / 5
    ...
    knn_dist = get_knn_dist(E, os.path.join(args.save_dir, args.model, 'all_imgs'), "celeba_private_feats")
    print("KNN Dist %.2f" % knn_dist)

    print("=> Calculate the FID.")
    fid = calc_fid(recovery_img_path=os.path.join(args.save_dir, args.model, "success_imgs"),
