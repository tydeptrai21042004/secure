...
def prepare_results_dir(args):
    ...
    root = os.path.join(args.results_root,
                        args.data_name, args.target_model, args.eps)
...
def get_args():
    ...
    parser.add_argument('--target_model', type=str, help='VGG16 | IR152 | FaceNet64 |DP_SGD | NON_DP')
...
help='Dimension of generator input noise. default: 128')
    ...
    parser.add_argument('--checkpoint_interval', '-ci', type=int, default=200,
...
help='Interval of showing losses. default: 100')
    ...
    parser.add_argument('--cGAN', action='store_true', default=True, help='cal FID for cGAN')
...
def main():
    ...
    if args.target_model.startswith("VGG16"):
        ...
        target_model_path = 'E:/DO_AN/LPG-MI/checkpoints/target_model/VGG16_88.26.tar'
    if args.target_model.startswith("DP_SGD"):
        target_model = VGG16(args.num_classes)
        target_model = ModuleValidator.fix(target_model)
        target_model_path = './checkpoints/target_model/ghost_mixed.tar'
    if args.target_model.startswith("NON_DP"):
        target_model = VGG16(args.num_classes)
        target_model = ModuleValidator.fix(target_model)
        target_model_path = './checkpoints/target_model/non-DP.tar'


    target_model = torch.nn.DataParallel(target_model).to(device)
    target_model.load_state_dict(torch.load(target_model_path, map_location=device)['state_dict'], strict=False)
    target_model.eval()
